...
def detect_vulnerable_code(file_path):
    ...
    input_pattern = r'\$(\w+)\s*=\s*sanitize_text_field\s*\(\s*(?:\$_GET|\$_REQUEST)\s*\[(?:\'|\")(\w+)(?:\'|\")\]\s*\)'
    output_pattern = r'<input [^>]*value=[\'"]<\?php echo \$'
    ...
    for line_no, line in enumerate(content, start=1):
        if not line.strip().startswith('//') and not line.strip().startswith('/*') and not line.strip().startswith('*') and not line.strip().startswith('*/'): 
            sanitized_input_matches = re.finditer(input_pattern, line)
            for match in sanitized_input_matches:
                ...
                if plugin_installations(installations_count) >= int(install):
                    for input_line_no, input_line, output_line_no, output_line in vulnerable_lines:
                        if output_line_no > input_line_no:
                            ...
                            print()
                            if 'type' and 'text' in output_line:
                                print(ConsoleColor.OKCYAN +'Payload : \n'+ ConsoleColor.ENDC + ConsoleColor.BOLD +'" onfocus=javascript:alert(/XSS/); autofocus="'+ ConsoleColor.ENDC)
                                print()
                            elif 'type' and 'hidden' in output_line:
                                print(ConsoleColor.OKCYAN +'Payload : \n'+ ConsoleColor.ENDC + ConsoleColor.BOLD +'" accesskey=x onclick=javascript:alert(/XSS/); a="'+ ConsoleColor.ENDC)
                                print()
